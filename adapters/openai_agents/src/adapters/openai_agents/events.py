"""Protocol Event builders for OpenAI Agents SDK adapter hooks (T13.17).

Keeps ``tool_invocation`` / ``context_attestation`` shapes aligned with
``proto/adapter.proto`` and ``internal/protocol`` without pulling the heavy
``openai-agents`` PyPI package.
"""
