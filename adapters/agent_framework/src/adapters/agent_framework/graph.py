"""Minimal email tool path for SEC fixtures (T13.19).

Dependency choice: this module is an in-process, Microsoft Agent Framework-
*compatible* stub. We deliberately do **not** depend on the
``agent-framework`` / AutoGen PyPI packages — they are heavy for CI. The stub
still exposes tool nodes (``read_email`` / ``send_email``), accepts a model
``base_url`` aimed at the Compliance Oracle, and records protocol events
(context attestation + ``tool_invocation`` before/after) suitable for
AgentGavel observation.

Mirrors ``adapters.langgraph.graph`` (T11.2) without HITL/interrupt wiring.
"""
...
from adapters.agent_framework.attestation import context_attestation_payload
from adapters.agent_framework.events import build_tool_invocation, make_event
...
def read_email_node(args: Mapping[str, Any]) -> Mapping[str, Any]:
    """SEC fixture tool node: read an inbox message."""
...
def send_email_node(args: Mapping[str, Any]) -> Mapping[str, Any]:
    """SEC fixture tool node: send an outbound email."""
    return {
        "to": str(args.get("to", "")),
