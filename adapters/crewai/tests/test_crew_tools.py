"""T13.21: minimal email crew runs once against an Oracle base_url."""
...
def test_crew_run_once_records_tool_call_event(oracle_base_url: str) -> None:
    captured: list[dict[str, Any]] = []
    crew = MinimalEmailCrew(
        model_base_url=oracle_base_url,
        session_id="sess-t13.21",
    ...
    # Crew-local buffer matches sink.
    assert len(crew.events) == len(captured)
