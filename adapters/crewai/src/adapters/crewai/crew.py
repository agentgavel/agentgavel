"""Minimal email tool path for SEC fixtures (T13.21).

Dependency choice: this module is an in-process, CrewAI-*compatible* stub.
We deliberately do **not** depend on the ``crewai`` PyPI package — it pulls
a large transitive tree unsuitable for CI. The stub still exposes tool nodes
...
from adapters.crewai.attestation import context_attestation_payload
from adapters.crewai.events import build_tool_invocation, make_event
...
class MinimalEmailCrew:
    """One-pass crew: Oracle completion → tool node → recorded events."""

    def __init__(
        self,
        *,
        model_base_url: str,
        session_id: str = "crewai-crew-sess",
    ...
    def run(
        ...
        *,
        ...
        """Run the crew once against the Oracle at ``model_base_url``."""
        directive = (
