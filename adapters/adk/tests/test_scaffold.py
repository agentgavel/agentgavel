...
def test_handshake_provenance_unofficial() -> None:
    report = AdkAdapter().handshake("1.0", engine_version="0.0.0-dev")
    assert report["adapter_name"] == "adk"
    assert report["framework_name"] == "google-adk"
...
def test_module_help_exits_zero() -> None:
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "adapters.adk", "--help"],
        capture_output=True,
        text=True,
        env=env,
        check=False,
    ...
    assert "usage" in proc.stdout.lower() or "ADK adapter" in proc.stdout
