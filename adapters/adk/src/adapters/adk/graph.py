"""Minimal email tool graph for SEC fixtures (T13.16).

Dependency choice: this module is an in-process, Google-ADK-*compatible* stub.
We deliberately do **not** depend on the ``google-adk`` PyPI package — it is
heavy for CI. The stub still exposes tool nodes (``read_email`` / ``send_email``),
accepts a model ``base_url`` aimed at the Compliance Oracle, and records
protocol events (context attestation + ``tool_invocation`` before/after)
suitable for AgentGavel observation.
"""
...
from adapters.adk.attestation import context_attestation_payload
from adapters.adk.events import build_tool_invocation, make_event
