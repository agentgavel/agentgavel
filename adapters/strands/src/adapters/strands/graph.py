"""Minimal email tool graph for SEC fixtures (T13.20).

Dependency choice: this module is an in-process, AWS Strands-*compatible* stub.
We deliberately do **not** depend on the ``strands-agents`` PyPI package — it is
heavy for CI. The stub still exposes tool nodes (``read_email`` / ``send_email``),
accepts a model ``base_url`` aimed at the Compliance Oracle, and records protocol
events (context attestation + ``tool_invocation`` before/after) suitable for
AgentGavel observation.

HITL / interrupt mapping is out of scope for T13.20 (CapabilityReport.hitl stays
false).
"""
...
from adapters.strands.attestation import context_attestation_payload
from adapters.strands.events import build_tool_invocation, make_event
...
def invoke_tool_node(tool_name: str, arguments: Mapping[str, Any]) -> Mapping[str, Any]:
    """Execute a registered tool node by name."""
