"""Protocol Event builders for AWS Strands adapter hooks (T13.20).

Keeps ``tool_invocation`` / ``context_attestation`` shapes aligned with
``proto/adapter.proto`` and ``internal/protocol`` (mirrors LangGraph T11.2/T11.4).
"""
...
def build_gate_decision(
    ...
    *,
    ...
    """Build a ``gate_decision`` payload (reserved for a future HITL path)."""
    gate: dict[str, Any] = {
