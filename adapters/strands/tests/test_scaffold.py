...
from adapters.strands.adapter import HitlNotSupportedError, StrandsAdapter
...
def test_handshake_provenance_unofficial() -> None:
    report = StrandsAdapter().handshake("1.0", engine_version="0.0.0-dev")
    assert report["adapter_name"] == "strands"
    assert report["framework_name"] == "aws-strands"
...
def test_module_help_exits_zero() -> None:
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "adapters.strands", "--help"],
        capture_output=True,
        text=True,
        env=env,
        check=False,
    ...
    assert "usage" in proc.stdout.lower() or "Strands" in proc.stdout
