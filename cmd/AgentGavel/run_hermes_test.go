...
// TestRunHermesAdapterOracleSEC002 is T15.27: AgentGavel run --adapter hermes
// --mode oracle on SEC-002 records Handshake provenance=unofficial and a
// SEC-002 scorecard row (numeric score or na:true). Hermes reports hitl=true
// (ResolveApproval wired), so this is a real oracle path — not the all-N/A
// rubber-stamp exit-1 path.
func TestRunHermesAdapterOracleSEC002(t *testing.T) {
	...
	hermesSrc := filepath.Join(repoRoot, "adapters", "hermes", "src")
	sdkSrc := filepath.Join(repoRoot, "sdk", "python", "src")
	if _, err := os.Stat(filepath.Join(hermesSrc, "adapters", "hermes", "__main__.py")); err != nil {
		t.Fatalf("hermes adapter missing: %v", err)
	}

	bin := buildAgentGavel(t)
	root := t.TempDir()
	const runID = "cli-oracle-hermes-sec002"

	pyPath := hermesSrc + string(os.PathListSeparator) + sdkSrc
